...
const Capsules = () => {
    ...
    return (
        <div className="max-h-full w-full py-16 bg-[#BFB6A0] px-20 space-y-4">
            ...
            <div className="flex space-x-4 justify-end">
                <button onClick={() => { currentPage > 1 && setCurrentPage(prev => prev - 1) }}
                    ...
                    className={`${currentPage === 1 ? 'bg-gray-400 text-gray-700' : 'bg-[#093247] text-white'}  px-2 py-1 rounded-md font-semibold text-sm`}>
                    PREV
                </button>
                <button onClick={() => { currentPage < numOfPages && setCurrentPage(prev => prev + 1) }}
                    disabled={currentPage === numOfPages}
                    className={`${currentPage === numOfPages ? 'bg-gray-400 text-gray-700' : 'bg-[#093247] text-white'}  px-2 py-1 rounded-md font-semibold text-sm`}>
                    NEXT
                </button>
            </div>
